...
```
対策: 適切な認証・認可を行い、権限のないユーザーがアクセスできないようにします。<br>

2. クロスサイトスクリプティング (XSS)<br>
...
3. クロスサイトリクエストフォージェリ (CSRF)<br>
APIを不正に呼び出されるリスクがあります。<br>
対策: CSRFトークンを使用して、APIのリクエストが正当なものか確認します。<br>
...
```
対策: パラメータ化クエリを使用し、またはORMツールを使用してデータアクセスを行います。<br>
...
```
対策: 必要なオリジンのみを許可するようにCORSポリシーを設定します。<br>